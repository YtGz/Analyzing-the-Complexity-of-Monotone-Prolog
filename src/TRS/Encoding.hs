module TRS.Encoding where
  ...
  encodeIn (BNode (([(ts,_,_)], (g,_)), "instance") l@(BNode (([(_,mu,_)],_),"instanceChild") Empty Empty) _) = fmap (apply mu) (encodeIn l)
  encodeIn (BNode (([(ts,_,_)], (g,_)), _) _ _) = do
    i <- get
    modify (+1)
    return (Fun ("fin" ++ show i) (concatMap (map Var . vars) ts `intersect` g))
  encodeIn _ = error "Cannot encode abstract state: multiple goals."

  encodeOut :: BTree (AbstractState, String) -> StateT Int IO Term'
  encodeOut (BNode (([(ts,_,_)], (g,_)), "instance") l@(BNode (([(_,mu,_)],_),"instanceChild") Empty Empty) _) = fmap (apply mu) (encodeOut l)
